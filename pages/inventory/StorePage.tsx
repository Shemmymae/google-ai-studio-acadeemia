...
import { getStores, addStore, deleteStore, Store, getSchools, School } from '../../db';
...
const StoreList = ({ stores, onStoreDeleted }: { stores: Store[], onStoreDeleted: () => void }) => {
        ...
        <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50 dark:bg-gray-700/50">
                  <tr>
                    {['SI', 'School', 'Store Name', 'Store Code', 'Mobile No', 'Address', 'Description', 'Action'].map(head => (
                ...
                <tbody>
                  {stores.map((store, index) => (
                    <tr key={store.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{index + 1}</td>
                      <td className="p-3 text-sm text-text-primary dark:text-gray-200">{store.school}</td>
                      <td className="p-3 text-sm text-text-primary dark:text-gray-200 font-medium">{store.name}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{store.code}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{store.mobile}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{store.address}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{store.description}</td>
                      <td className="p-3">
                        <div className="flex space-x-2">
                          <button className="text-blue-500 hover:text-blue-700 dark:hover:text-blue-400 transition-colors" aria-label="Edit store">{icons.edit}</button>
                          <button onClick={() => handleDelete(store.id)} className="text-red-500 hover:text-red-700 dark:hover:text-red-400 transition-colors" aria-label="Delete store">{icons.delete}</button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mt-6 text-sm text-text-secondary dark:text-gray-400">
                <div className="flex items-center gap-2 mb-4 md:mb-0">
                    <span>Showing 1 to {stores.length} of {stores.length} entries</span>
...
const CreateStoreForm = ({ onStoreAdded }: { onStoreAdded: () => void }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newStore: Omit<Store, 'id' | 'school_id'> = {
    ...
    return (
        <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField label="Store Name" name="name" required />
                <FormField label="Store Code" name="code" required />
                <FormField label="Mobile No" name="mobile" type="tel" required />
            </div>
            <FormField label="Address" name="address" />
            <FormField label="Description" name="description" type="textarea" placeholder="Description" rows={4}/>
      ...
      <div className="bg-card dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow-md">
        <div className="border-b dark:border-gray-700 mb-6">
          <nav className="flex space-x-2 -mb-px">
            <TabButton label="Store List" icon={icons.list} currentView={view} targetView="list" />
            <TabButton label="Create Store" icon={icons.add} currentView={view} targetView="create" />
          </nav>
        </div>
        {loading ? (
             <div className="text-center py-8 text-text-secondary dark:text-gray-400">Loading Stores...</div>
        ) : view === 'list' ? (
            <StoreList stores={stores} onStoreDeleted={fetchStores} />
        ) : (
            <CreateStoreForm onStoreAdded={handleStoreAdded} />
        )}
      </div>
    </DashboardLayout>
  );
