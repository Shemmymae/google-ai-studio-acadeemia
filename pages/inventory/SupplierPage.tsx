...
import { getSuppliers, addSupplier, deleteSupplier, Supplier, getSchools, School } from '../../db';
...
const SupplierList = ({ suppliers, onSupplierDeleted }: { suppliers: Supplier[], onSupplierDeleted: () => void }) => {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50 dark:bg-gray-700/50">
                  <tr>
                    {['SI', 'School', 'Supplier Name', 'Address', 'Contact Number', 'Email', 'Company Name', 'Product List', 'Action'].map(head => (
                ...
                <tbody>
                  {suppliers.map((supplier, index) => (
                    <tr key={supplier.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{index + 1}</td>
                      <td className="p-3 text-sm text-text-primary dark:text-gray-200">{supplier.school}</td>
                      <td className="p-3 text-sm text-text-primary dark:text-gray-200 font-medium">{supplier.name}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{supplier.address}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{supplier.contact}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{supplier.email}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{supplier.company}</td>
                      <td className="p-3 text-sm text-text-secondary dark:text-gray-400">{supplier.products}</td>
                      <td className="p-3">
                        <div className="flex space-x-2">
                          <button className="text-blue-500 hover:text-blue-700 dark:hover:text-blue-400 transition-colors" aria-label="Edit supplier">{icons.edit}</button>
                          <button onClick={() => handleDelete(supplier.id)} className="text-red-500 hover:text-red-700 dark:hover:text-red-400 transition-colors" aria-label="Delete supplier">{icons.delete}</button>
                        </div>
                      </td>
                    </tr>
                  ))}
...
const FormField = ({ label, name, type = 'text', required = false, placeholder = '', children, rows }: { label: string, name: string, type?: string, required?: boolean, placeholder?: string, children?: React.ReactNode, rows?: number }) => (
    <div>
...
const CreateSupplierForm = ({ onSupplierAdded }: { onSupplierAdded: () => void }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newSupplier: Omit<Supplier, 'id' | 'school_id'> = {
    ...
    return (
        <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField label="Supplier Name" name="name" required />
                <FormField label="Contact Number" name="contact" type="tel" required />
                <FormField label="Email" name="email" type="email" />
                <FormField label="Company Name" name="company" />
                <FormField label="Product List" name="products" />
            </div>
            <FormField label="Address" name="address" type="textarea" placeholder="Address" rows={4}/>
      ...
      <div className="bg-card dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow-md">
        <div className="border-b dark:border-gray-700 mb-6">
          <nav className="flex space-x-2 -mb-px">
            <TabButton label="Supplier List" icon={icons.list} currentView={view} targetView="list" />
            <TabButton label="Create Supplier" icon={icons.add} currentView={view} targetView="create" />
          </nav>
        </div>
        {loading ? (
             <div className="text-center py-8 text-text-secondary dark:text-gray-400">Loading Suppliers...</div>
        ) : view === 'list' ? (
            <SupplierList suppliers={suppliers} onSupplierDeleted={fetchSuppliers} />
        ) : (
            <CreateSupplierForm onSupplierAdded={handleSupplierAdded} />
        )}
      </div>
    </DashboardLayout>
  );
